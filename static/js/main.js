...
const username = helpers.getURLParam("username") || "OWLin";
const waifuName = helpers.getURLParam("waifuname") || "Eveline";
...
function onInteract(model, getInteraction) {
    ...
    if (synthesizer != null && synthesizer.speaking) { // check if synthesizer is speaking
    ...
    $transcription.dblclick(doubleClickEvent);
    ...
    function callback(userPrompt) {
        ...
        ai.accept(userPrompt, function (response, error) {
            ...
            // start transcription event if synthesizer is not speaking
